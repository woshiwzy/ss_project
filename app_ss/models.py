...
class RewardHistory(models.Model):
    uuid = models.CharField(max_length=60, default='', verbose_name="用户UUID", db_index=True)
    ...
    def to_dict(self):
        data = {}
        for f in self._meta.concrete_fields:
            if f.name == 'descption' or f.name == 'reward_size' or f.name == 'username':
                data[f.name] = f.value_from_object(self)
            if f.name == 'create_time':
